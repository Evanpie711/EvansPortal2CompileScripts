...
P2PATH="/home/USERNAME/.steam/debian-installation/steamapps/common/Portal 2 Community Edition/" #set to the portal 2 ce root directory
FINALMAPPATH="p2ce/" 
GAME="p2ce/" #change the game if needed
#USEWINE=0 #Set to one to use windows compilers though wine, otherwise keep 0 (Not done ig)
#
#-------------------------------------------------------------------------------------------------------------------
...
POSTCOMPILERCOMMAND="./bin/linux64/postcompiler/postcompiler"
...
#else
VBSPPATH="bin/linux64/vbsp"
VVISPATH="bin/linux64/vvis"
VARDPATH="bin/linux64/vrad"
#fi
...
VRAD_ARG_FULL=("-hdr" "-PortalTraversalAO" "-staticproppolys" "-staticproplighting")
...
VBSPCOMMAND=("./$VBSPPATH" -game "$GAMEDIR" "$MAPPATH")
echo -e "\e[32m---------------------------------------------------------------------"
echo "running bsp"
echo "${VBSPCOMMAND[*]}"
echo -e "---------------------------------------------------------------------\e[0m"
...
VVISCOMMAND=("./$VVISPATH")
VVISCOMMAND+=(${VVIS_ARG[@]})
VVISCOMMAND+=("-game" "$GAMEDIR" "$MAPPATH")
echo -e "\e[32m---------------------------------------------------------------------"
echo "running vvis"
echo "${VVISCOMMAND[*]}"
echo -e "---------------------------------------------------------------------\e[0m"
...
VRADCOMMAND=("./$VARDPATH")
VRADCOMMAND+=(${VRAD_ARG[@]})
VRADCOMMAND+=("-game" "$GAMEDIR" "$MAPPATH")
echo -e "\e[32m---------------------------------------------------------------------"
echo "running vrad"
echo "${VRADCOMMAND[*]}"
echo -e "---------------------------------------------------------------------\e[0m"
